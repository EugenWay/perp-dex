...
impl TradingModule {
    ...
    fn execute_market_order(caller: ActorId, params: CreateOrderParams) -> Result<ExecutionResult, Error> {
        let quote = match params.order_type {
            OrderType::MarketIncrease => PricingModule::quote_increase(&params.market, &params.side, params.size_delta_usd)?,
            OrderType::MarketDecrease => PricingModule::quote_decrease(&params.market, &params.side, params.size_delta_usd)?,
        ...
        Ok(ExecutionResult::Executed { position_key: key, execution_price: quote.execution_price })
    }

    fn execute_limit_order(caller: ActorId, params: CreateOrderParams) -> Result<ExecutionResult, Error> {
        let quote = match params.order_type {
            OrderType::LimitIncrease => PricingModule::quote_increase(&params.market, &params.side, params.size_delta_usd)?,
            OrderType::LimitDecrease | OrderType::StopLossDecrease => PricingModule::quote_decrease(&params.market, &params.side, params.size_delta_usd)?,
            _ => return Err(Error::UnsupportedOrderType),
        };
    ...
    pub fn execute_saved_order(executor: ActorId, key: RequestKey) -> Result<ExecutionResult, Error> {
        ...
        let quote = match order.order_type {
            OrderType::LimitIncrease => PricingModule::quote_increase(&order.market, &params.side, params.size_delta_usd)?,
            OrderType::LimitDecrease | OrderType::StopLossDecrease => PricingModule::quote_decrease(&order.market, &params.side, params.size_delta_usd)?,
            _ => return Err(Error::UnsupportedOrderType),
        };

        Self::validate_execution_price(&params, quote.execution_price)?;
        let position_key = Self::execute_position_change(order.account, &params, quote.execution_price)?;
        ...
        let om = st.orders.get_mut(&key).unwrap();
        om.status = OrderStatus::Executed;
        om.updated_at_block = exec::block_height();
        om.updated_at_time = exec::block_timestamp();

        Ok(ExecutionResult::Executed { position_key, execution_price: quote.execution_price })
    }
    ...
    pub fn update_order(caller: ActorId, key: RequestKey, params: UpdateOrderParams) -> Result<(), Error> {
        ...
        if let Some(v) = params.size_delta_usd     { o.size_delta_usd = v; }
        if let Some(v) = params.trigger_price      { o.trigger_price = v; }
        if let Some(v) = params.acceptable_price   { o.acceptable_price = v; }

        o.updated_at_block = exec::block_height();
        o.updated_at_time  = exec::block_timestamp();
        Ok(())
    }
    ...
    fn validate_order_params(p: &CreateOrderParams) -> Result<(), Error> {
        if p.size_delta_usd == 0 { return Err(Error::InvalidOrderSize); }
        if p.acceptable_price == 0 { return Err(Error::InvalidPrice); }
        if matches!(p.order_type, OrderType::LimitIncrease | OrderType::LimitDecrease | OrderType::StopLossDecrease) && p.trigger_price == 0 {
            return Err(Error::InvalidTriggerPrice);
        }
        if matches!(p.order_type, OrderType::MarketIncrease | OrderType::LimitIncrease) && p.collateral_delta_amount == 0 {
    ...
    fn can_execute_limit_order(p: &CreateOrderParams, current_price: u128) -> bool {
        ...
        match p.order_type {
            OrderType::LimitIncrease => if is_long { current_price <= p.trigger_price } else { current_price >= p.trigger_price },
            OrderType::LimitDecrease => if is_long { current_price >= p.trigger_price } else { current_price <= p.trigger_price },
            OrderType::StopLossDecrease => if is_long { current_price <= p.trigger_price } else { current_price >= p.trigger_price },
            _ => false,
        }
    }
